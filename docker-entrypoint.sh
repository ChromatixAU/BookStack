...
if [ ! -f "${BOOKSTACK_HOME}/.env" ]; then
  ...
  cat > "${BOOKSTACK_HOME}/.env" <<EOF
      ...
      # Mail settings
      MAILGUN_DOMAIN=${MAILGUN_DOMAIN:-false}
      MAILGUN_SECRET=${MAILGUN_SECRET:-false}
      MAIL_FROM=${MAIL_FROM:-bookstack@chromatix.com.au}
...
hostname > "${BOOKSTACK_HOME}/public/host.html"
...
chown www-data:www-data storage && chmod 775 storage
